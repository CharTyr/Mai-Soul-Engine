...
如果你是“新接手的 AI”，请先读：`docs/交接指南_AI.md`。
...
1) **禁止修改 MaiBot 主程序**（`MaiBot/src/**`）。只能改插件目录。  
2) **不能阻塞 Dream**：所有耗时操作要快进快出，失败要兜底。  
3) **隐私脱敏**：不要输出原句复刻/链接/号码/邮箱/可识别个人信息。  
4) **API 契约优先**：对照 `前端需求.md`，字段名/单位/枚举不可随意破坏。  
5) **global 模式要护栏**：白名单 + 每日限额 + 最小能量/置信度门槛。  
...
`plugin.py` 是唯一实现文件。建议你用 `rg` 直接搜索以下函数名：

Dream：
- `try_hook_dream()` / `_on_dream_cycle()`

Spectrum（Dream window）：
- `on_message()` / `_append_pending_message_locked()`
- `_dream_spectrum_window_sweep()` / `_process_spectrum_window_for_group()`
- `_semantic_shift_window()` / `_apply_window_analysis()`

Cabinet（ideology_lab）：
- topics→slots：在 `_apply_window_analysis()` 中
- `_mastication_sweep_once()` / `_mastication_once()`
- `_evaluate_slot_with_llm()`（或按 `rg "request_type=\\\"mai_soul_engine\\.cabinet"` 搜 evaluator prompt）
- `_assimilate_slot_locked()` / `_finalize_slot_locked()`（按 `rg \"assimil\"`）
- `rethink`：`_rethink_sweep_once()` / `_rethink_next_global_trait()`

Blackbox：
- `_blackbox_sweep()` / `_generate_and_store_fragment()`
- dream_trace：`_append_trace_locked()`
- 合并输出：`_snapshot_fragments_frontend()`

Injection：
- `build_injection_block()`
- 6 档曲线：`_axis_tier()` / `_axis_instruction()` / `_spectrum_to_instructions()`
- 相关 trait：`_select_relevant_traits()`

API：
- `setup_router()` / `_resolve_stream_id()`
- `_snapshot_*_frontend()`（对齐 `前端需求.md`）
- `_require_api_token()` / `_require_mutation_safety()`

Persistence：
- `_persist_unlocked()` / `load_persisted()`（`data/state.json`）
...
### 10.1 state.json 结构（概览）

文件位置：`plugins/MaiBot_Soul_Engine/data/state.json`

高层结构（概念化）：
- `global`：
  - `base_offset`：全局底色偏移（四轴）
  - `traits`：全局固化 traits（最近 N 条）
  - `daily_used_abs`：global 每日限额消耗
- `groups[stream_id]`：
  - `values/ema/base_tone/recent_deltas`
  - `pending_messages/last_dream_processed_ts`
  - `slots/traits/dissonance`
  - `fragments/dream_trace`
  - `influences/tag_cloud/quote_bank/prototypes`

注意：
- 这是“人格硬盘”，生产环境建议定期备份。
- 所有列表都必须 cap，否则会导致 state.json 膨胀。
