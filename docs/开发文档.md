...
相关性选择：使用轻量检索（BM25 + 旧思想温和衰减）做**确定性 TopN**；不依赖 LLM 是否调用工具。  
实现入口：`SoulEngine.build_injection_block()`。

配置：
- `injection.enabled`
- `injection.policy`（auto/full/spectrum_only）
- `injection.min_thought_score`
- `injection.max_thought_details`
- `injection.max_chars`
- `performance.reply_prompt_budget_chars` / `performance.reply_injection_safety_chars`（注入预算）

### 5.1 注入命中可视化

插件会记录最近一次注入命中（不落盘用户原句），用于前端展示：  
`GET /api/v1/soul/injection`

### 5.2 主程序记忆检索工具（Memory Retrieval tools）

插件启动时会注册 2 个检索工具（不改主程序代码）：
- `soul_search_thoughts(query, top_k?, min_score?)`：检索“固化思想”
- `soul_get_thought(thought_id)`：获取思想详情（定义/结论/影响）

用途：当主程序进入 Memory Retrieval 链路时，LLM 可调用这些工具拿到“固化思想”，用于回答“你之前的观点/立场是什么”一类问题。
...
### 7.1 schema_version 与迁移

`state.json` 带 `schema_version`，加载时会自动迁移到当前版本；同时支持 `.bak` 回滚与损坏文件保全（`.corrupt.*`）。

### 7.2 导入/导出（迁移/备份）

- 导出（HTTP，只读、脱敏、不含 pending_messages 全文）：`GET /api/v1/soul/export`
- 导入（仅本地文件，默认关闭）：
  - 把导出的 JSON 放到 `data/import.json`（或 `persistence.import_path` 指定路径）
  - `persistence.import_on_start=true` 后重启 MaiBot
  - 导入成功会把文件改名为 `.imported.<ts>`

### 7.3 审计日志（Ops）

`data/audit.jsonl`：记录调试命令、导入等运维操作（不落盘原始 user_id；只写哈希化 uid）。支持大小轮转。
