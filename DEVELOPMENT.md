...
- 配置：
  ...
  - 字段映射：
    ...
    - spectrum：`notion.spectrum_property_*`（`dimension_rows` 默认 `Dimension/ScopeId/Value`）
- 去重/关联：以 `TraitId` 字段为主键（`rich_text equals trait_id`），并在 `data/notion_frontend_state.json` 缓存 `trait_id -> page_id` 映射，减少 query 成本。
- 光谱 upsert：
  - `dimension_rows`：以 `ScopeId + Dimension(title)` 为主键，维护 `Value/UpdatedAt`（并在可选字段存在时写入）。
  - `single_row`：以 `ScopeId` 为主键，维护 `Economic/Social/Diplomatic/Progressive/Initialized/LastEvolution/UpdatedAt`。
- “永不覆盖可编辑字段”：
