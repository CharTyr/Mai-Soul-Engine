...
- **配置**：新增 `[self_reflection]` 段（enabled/evaluation_interval_hours/max_replies_per_cycle/self_reflection_weight/auto_internalize_threshold/user_reaction_signal_enabled/relevance_gate_enabled/normalize_across_batch/pending_max_age_hours/pending_max_rows）；`CONFIG_VERSION` 升至 `2.3.0`，manifest 同步 `2.3.0`，`config_template.toml` 同步。
- **自指风险护栏**：OBSERVE 不改写输出 / 评价异步批量有 dead zone / weight<1 / strengthened trait 豁免 / self_observation trait 默认全人工审批 / 评估 prompt 不给标准答案+对立视角 / 批次归一化可选 / 反向采样（最低分强制进审核，`auto_internalize_threshold>0` 时）。
