...
- `tests/` — 约 **600** 项（宿主根 `uv run pytest plugins/CharTyr_Mai-Soul-Engine/tests/ -q`）；覆盖宿主契约、快照配对（**含内容证据/弃权规则**）、种子保留、槽位恢复、操作租约与队列、卸载隔离、通知 outbox、运行模式与命令闸门、候选校验、任务监督、会话类型、迁移盘点、**迁移鲁棒性**（WAL / 损坏库 / 中断 / 重复）、**平台探测**、**隐私与保留期**、**离线回放** 等
