...
| `models/` | 按实体拆分：`_conn.py`（连接/建表/迁移）、`spectrum.py`/`history.py`/`seeds.py`/`traits.py`/`p1.py`（各实体 CRUD）；`ideology_model.py` 为重导出 shim，40+ 处历史导入零破坏。P1 相关：+3 表（slices/mood/edges）+2 列（`ideology_layer`/`lifecycle_state`）+ 列重命名迁移 |
