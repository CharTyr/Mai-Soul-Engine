...
只读详情：`/soul_seed <id>`（种子详情含上下文）、`/soul_trait <id>`（trait 详情含分层/生命周期/图谱边，含 `contradicted_by`/`weakened_by`/`revised_by` 关系边）。批量：`/soul_reject_all`（**只有批量拒绝，无批量批准**——批准会触发大量 LLM 内化）。误判为 contradicted 的 trait 可用 `/soul_trait_enable <id>` 重新启用。
...
- `tests/` — 插件内测试（28 项，从宿主根 `uv run pytest plugins/CharTyr_Mai-Soul-Engine/tests/ -q` 运行）；覆盖原子守卫/生命周期 setter/全局标记/批量边/质量分/清理 expired/矛盾排除
