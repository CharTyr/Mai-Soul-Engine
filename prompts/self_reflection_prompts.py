...
_RELEVANCE_GATE = """\
回复相关性三档（决定是否打分及打分深度）：
- social_glue（纯闲聊社交，不打分）：如"哈哈""对啊""好的""嗯嗯""666"、表情、打招呼、纯附和、纯礼貌（谢谢/不客气）、复读刷梗。
- reactive（有反应但没表态，只评语气）：接话、回应但没站立场、没给信息或决策。如"我知道了""是这样""哦"、反问（"那你觉得呢？"）、吐槽/阴阳怪气（有情绪但无明确立场）。
- substantive（真正表态/决策，完整评）：含明确表态（我认为/我觉得/建议/不赞同）、信息性回答（给事实/解释/分析）、决策建议（应该/可以/最好）、拉架/转移话题（有意图的社交行为）。\
"""
