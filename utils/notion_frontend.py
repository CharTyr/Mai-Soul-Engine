...
def build_notion_frontend_config(plugin, *, section: str = "notion") -> NotionFrontendConfig:
    # 兼容新旧配置方式：优先使用 plugin.config.notion（maibot_sdk 2.x），
    # 否则回退到 plugin.get_config（旧版兼容）
    if hasattr(plugin, "config") and hasattr(plugin.config, section):
        cfg_section = getattr(plugin.config, section)

        def _get(key: str, default=None):
            return getattr(cfg_section, key, default)

    else:
        def _get(key: str, default=None):  # type: ignore[misc]
            return plugin.get_config(f"{section}.{key}", default)

    enabled = bool(_get("enabled", False))

    token_cfg = str(_get("token", "") or "").strip()
    token = token_cfg or os.getenv(NOTION_ENV_TOKEN, "").strip()

    database_id = _normalize_notion_id(str(_get("database_id", "") or ""))

    sync_spectrum = bool(_get("sync_spectrum", True))
    spectrum_database_id = _normalize_notion_id(str(_get("spectrum_database_id", "") or ""))
    spectrum_scope_id = str(_get("spectrum_scope_id", "global") or "global").strip() or "global"
    spectrum_mode = str(_get("spectrum_mode", "dimension_rows") or "dimension_rows").strip()

    sync_interval_seconds = max(60, int(_get("sync_interval_seconds", 600)))
    first_delay_seconds = max(0, int(_get("first_delay_seconds", 5)))
    max_traits = max(0, int(_get("max_traits", 200)))
    ...
    property_map = NotionPropertyMap(
        ...
        trait_id=str(_get("property_trait_id", "TraitId") or "TraitId"),
        tags=str(_get("property_tags", "Tags") or "Tags"),
        question=str(_get("property_question", "Question") or "Question"),
        thought=str(_get("property_thought", "Thought") or "Thought"),
        confidence=str(_get("property_confidence", "Confidence") or "Confidence"),
        impact_score=str(_get("property_impact_score", "ImpactScore") or "ImpactScore"),
        status=str(_get("property_status", "Status") or "Status"),
        visibility=str(_get("property_visibility", "Visibility") or "Visibility"),
        updated_at=str(_get("property_updated_at", "UpdatedAt") or "UpdatedAt"),
    )
    ...
    spectrum_property_map = NotionSpectrumPropertyMap(
        ...
        updated_at=str(_get("spectrum_property_updated_at", "UpdatedAt") or "UpdatedAt"),
    )
...
def sync_traits_to_notion(*, plugin_dir: Path, cfg: NotionFrontendConfig) -> dict[str, Any]:
    ...
    # 查询全部 traits（含已删除/已禁用，用于同步状态到 Notion）
    traits = query_crystallized_traits(deleted=False, order_by_created_desc=True, limit=cfg.max_traits or 100000)
